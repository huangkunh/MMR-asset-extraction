...
> **以下方法尚未通过人工验证，存在已知问题，未发布提取素材。需经验证后方可标记为已验证。**

| 方法 | 状态 | 问题描述 |
|------|------|----------|
| VRAM 字节提取法（新方法） | **待验证** | tilemap渲染时瓦片索引与字符基地址偏移不正确，各场景BG1 screen渲染结果几乎相同，未正确还原游戏画面。SNES PPU字符基地址寄存器($210B)为只写，无法通过Mesen2读取，导致无法确定正确的tile-to-VRAM偏移关系。|
